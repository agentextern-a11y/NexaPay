...
import { ArrowUpRight, ArrowDownRight, Activity, Wifi, CreditCard, TrendingUp, Send, Wallet, Download } from "lucide-react";
...
function CryptoLogo({ symbol, color, size = 7 }: { symbol: string; color?: string; size?: number }) {
  ...
  if (url && !err) {
    return (
      <div className={`${dim} rounded-full overflow-hidden bg-white border border-border/30 flex items-center justify-center flex-shrink-0`}>
        <img
          src={url}
          alt={symbol}
  ...
  return (
    <div
      className={`${dim} rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0`}
...
const ACTIVITY_ICONS: Record<string, React.ReactNode> = {
  ...
  receive:    <ArrowDownRight className="h-4 w-4 text-emerald-600" />,
  swap:       <ArrowUpRight  className="h-4 w-4 text-blue-500" />,
...
function EmptyWalletState({ ownerName }: { ownerName: string }) {
  return (
    ...
    >
      ...
      <p className="text-muted-foreground mb-2 max-w-sm leading-relaxed text-base">
        Your cryptographic keys have been generated. Deposit crypto to your real on-chain addresses to get started.
      </p>
      <p className="text-xs text-muted-foreground/60 mb-8 max-w-sm">
        BTC, ETH, SOL, BNB & USDC addresses are live — no demo data.
      </p>
      <div className="flex flex-wrap justify-center gap-3">
        <Link href="/wallet">
          <Button className="bg-primary text-primary-foreground hover:bg-primary/90 gap-2 px-6 py-2.5">
            <Download className="h-4 w-4" /> Deposit Crypto
          </Button>
        </Link>
        <Link href="/send">
          <Button variant="outline" className="gap-2 px-6 py-2.5 border-border/60">
      ...
      <div className="mt-12 grid grid-cols-3 gap-4 max-w-sm w-full">
        {[
          ...
          <div key={f.label} className="flex flex-col items-center gap-2 p-3 rounded-xl bg-card/50 border border-border/40">
...
export default function Dashboard() {
  ...
  // Show empty state for new zero-balance wallets
  const isEmptyWallet =
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Welcome, {ownerName}</p>
          </div>
        </div>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
      ...
      {summaryLoading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          <Card className="col-span-2 bg-gradient-to-br from-primary/10 via-card/80 to-card/50 border-primary/20 backdrop-blur-xl relative overflow-hidden" data-testid="card-total-balance">
            ...
            <CardContent className="p-6">
              ...
              <div className={`mt-2 text-sm font-medium flex items-center gap-1 ${Number(summary.changePct24h) >= 0 ? "text-emerald-600" : "text-red-600"}`}>
          ...
          <Card className="bg-card/50 border-border/50 backdrop-blur-xl" data-testid="card-active-assets">
          ...
          <Card className="bg-card/50 border-border/50 backdrop-blur-xl" data-testid="card-nfc-payments">
            <CardContent className="p-5">
        ...
        <Card className="bg-card/50 border-border/50 backdrop-blur-xl">
          ...
          <CardContent className="space-y-3">
            {assetsLoading ? (
              ...
              <p className="text-xs text-muted-foreground text-center py-4">No assets yet — deposit to get started</p>
              ...
              <div key={asset.id} className="flex items-center justify-between" data-testid={`holding-row-${asset.id}`}>
                ...
                <div className="text-right">
                  ...
                  <div className={`text-xs ${Number(asset.changePct24h) >= 0 ? "text-emerald-600" : "text-red-600"}`}>
          ...
          <CardContent className="space-y-3">
            {topMovers.map((coin) => (
              <div key={coin.symbol} className="flex items-center justify-between" data-testid={`mover-${coin.symbol}`}>
                ...
                <Badge variant="outline" className={`text-xs font-semibold ${Number(coin.changePct24h) >= 0 ? "text-emerald-700 border-emerald-300 bg-emerald-50" : "text-red-600 border-red-300 bg-red-50"}`}>
